...
#include "../Reflection/StateDefinitions.hpp"
...
public:
    static TupleBuffer serialize(const T& state, AbstractBufferProvider* bufferProvider) {
        // Use reflect-cpp with Cap'n Proto serialization
    ...
    static rfl::Result<T> deserialize(const TupleBuffer& buffer) {
        // Use reflect-cpp with Cap'n Proto deserialization
    ...
    // Zero-copy view - just deserialize for now, Cap'n Proto is already efficient
