...
public:
    ...
    OffsetBasedHashMap(uint64_t keySize, uint64_t valueSize, uint64_t bucketCount)
        ...
        static_assert(OffsetEntry::headerSize() == 16, "OffsetEntry header size must be 16 bytes");
        static_assert(sizeof(OffsetEntry) == 16, "OffsetEntry size must be 16 bytes");
...
private:
    ...
    uint32_t allocateEntry() {
        ...
        if (offset == 0) {
            // Skip offset 0 as it's used as nullptr sentinel
            // Start first entry at proper aligned offset after header
