...
/// State structure for serializable PagedVector - compatible with reflect-cpp
...
public:
    ...
    PagedVectorState& extractState() { return state_; }
    const PagedVectorState& extractState() const { return state_; }
    ...
    [[nodiscard]] uint64_t getNumberOfPages() const { return state_.pages.size(); }
...
private:
    PagedVectorState state_;
    
    // Cached TupleBuffers for compatibility (created on demand)
    mutable std::vector<std::unique_ptr<TupleBuffer>> cachedBuffers_;
    mutable std::unique_ptr<MemoryLayout> cachedMemoryLayout_;
