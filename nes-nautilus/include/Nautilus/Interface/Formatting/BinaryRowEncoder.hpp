...
// Forward declare ExecutionContext from NES runtime to avoid hard dependency in header
namespace NES { struct ExecutionContext; }
...
public:
    ...
    // Compute encoded size in bytes for a record.
    nautilus::val<uint32_t> computeSize(Record& record) const;

    // Encode a record to the provided destination buffer. The buffer must have at least computeSize() bytes.
