...
{
    // Write to file for debugging - this should definitely work
    FILE* debugFile = fopen("/tmp/nes_debug.log", "a");
    if (debugFile) {
        fprintf(debugFile, "REWRITE_RULE_DEBUG: Creating WindowedAggregation rewrite rule with enableSerializableAggregation=%s\n", 
                argument.conf.enableSerializableAggregation.getValue() ? "true" : "false");
        fclose(debugFile);
    }
    
    printf("REWRITE_RULE_DEBUG: Creating WindowedAggregation rewrite rule with enableSerializableAggregation=%s\n", 
           argument.conf.enableSerializableAggregation.getValue() ? "true" : "false");
    fflush(stdout);
