...
TEST_F(QueryEngineTest, singleQueryWithShutdown)
{
    ...
    test.start();
    {
        ...
        ctrl->injectData(std::vector(DEFAULT_BUFFER_SIZE, std::byte(0)), NUMBER_OF_TUPLES_PER_BUFFER);
        ctrl->injectData(std::vector(DEFAULT_BUFFER_SIZE, std::byte(0)), NUMBER_OF_TUPLES_PER_BUFFER);
        (void)test.sinkControls[pipeline]->waitForNumberOfReceivedBuffersOrMore(4);
...
TEST_F(QueryEngineTest, singleQueryWithManySources)
{
    ...
    test.start();
    {
        ...
        (void)sinkCtrl->waitForNumberOfReceivedBuffersOrMore(numberOfBuffersBeforeTermination);
        dataGenerator.stop();
...
TEST_F(QueryEngineTest, ManyQueriesWithTwoSources)
{
    ...
    test.start();
    {
        ...
        (void)sinkCtrls[0]->waitForNumberOfReceivedBuffersOrMore(2);
        dataGenerator.stop();
